...
#############       VPC Configurations      #############
        ...
        availabilityZonesList = [parameters_copy.az1, parameters_copy.az2]
        vpc = ec2.Vpc (self, "vpc",
            vpc_name = f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-vpc",
            ip_addresses = ec2.IpAddresses.cidr(parameters_copy.cidrRange),
            enable_dns_hostnames = parameters_copy.enableDnsHostnames,
            enable_dns_support = parameters_copy.enableDnsSupport,
            availability_zones = availabilityZonesList,
            nat_gateways = parameters_copy.numberOfNatGateways,
            subnet_configuration = [
                {
                    "name": f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-publicSubnet1",
                    "subnetType": ec2.SubnetType.PUBLIC,
                    "cidrMask": parameters_copy.cidrMaskForSubnets,
                },
                {
                    "name": f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-privateSubnet1",
                    "subnetType": ec2.SubnetType.PRIVATE_WITH_EGRESS,
                    "cidrMask": parameters_copy.cidrMaskForSubnets,
                }
            ]
        )
        tags.of(vpc).add("name", f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-vpc")
        tags.of(vpc).add("project", parameters_copy.project)
        tags.of(vpc).add("env", parameters_copy.env)
        tags.of(vpc).add("app", parameters_copy.app)

#############       EC2 Key Pair Configurations      #############

        keyPair = ec2.KeyPair.from_key_pair_name(self, "ec2KeyPair", parameters_copy.keyPairName)

#############       Security Group Configurations      #############

        sgEc2MskCluster = ec2.SecurityGroup(self, "sgEc2MskCluster",
            security_group_name = f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-sgEc2MskCluster",
            vpc=vpc,
            description="Security group associated with the EC2 instance of MSK Cluster",
            allow_all_outbound=True,
            disable_inline_rules=True
        )
        tags.of(sgEc2MskCluster).add("name", f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-sgEc2MskCluster")
        tags.of(sgEc2MskCluster).add("project", parameters_copy.project)
        tags.of(sgEc2MskCluster).add("env", parameters_copy.env)
        tags.of(sgEc2MskCluster).add("app", parameters_copy.app)

        sgMskCluster = ec2.SecurityGroup(self, "sgMskCluster",
            security_group_name = f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-sgMskCluster",
            vpc=vpc,
            description="Security group associated with the MSK Cluster",
            allow_all_outbound=True,
            disable_inline_rules=True
        )
        tags.of(sgMskCluster).add("name", f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-sgMskCluster")
        tags.of(sgMskCluster).add("project", parameters_copy.project)
        tags.of(sgMskCluster).add("env", parameters_copy.env)
        tags.of(sgMskCluster).add("app", parameters_copy.app)

        sgApacheFlink = ec2.SecurityGroup(self, "sgApacheFlink",
            security_group_name = f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-sgApacheFlink",
            vpc=vpc,
            description="Security group associated with the Apache Flink",
            allow_all_outbound=True,
            disable_inline_rules=True
        )
        tags.of(sgApacheFlink).add("name", f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-sgApacheFlink")
        tags.of(sgApacheFlink).add("project", parameters_copy.project)
        tags.of(sgApacheFlink).add("env", parameters_copy.env)
        tags.of(sgApacheFlink).add("app", parameters_copy.app)
        ...
        sgEc2MskCluster.add_ingress_rule(
            ...
            connection = ec2.Port.tcp_range(parameters_copy.sgKafkaInboundPort, parameters_copy.sgKafkaOutboundPort),
        ...
        sgMskCluster.add_ingress_rule(
            ...
            connection = ec2.Port.tcp_range(parameters_copy.sgMskClusterInboundPort, parameters_copy.sgMskClusterOutboundPort),
...
#############       S3 Bucket Configurations      #############
        ...
        bucket = s3.Bucket.from_bucket_name(self, "s3BucketAwsBlogArtifacts", parameters_copy.s3BucketName)

#############       KMS Configurations      #############

        customerManagedKey = kms.Key(self, "customerManagedKey",
            alias = f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-sasl/scram-key",
            description = "Customer managed key",
            enable_key_rotation = True,
            removal_policy = RemovalPolicy.DESTROY
        )
        tags.of(customerManagedKey).add("name", f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-customerManagedKey")
        tags.of(customerManagedKey).add("project", parameters_copy.project)
        tags.of(customerManagedKey).add("env", parameters_copy.env)
        tags.of(customerManagedKey).add("app", parameters_copy.app)
...
#############       Secrets Manager Configurations      #############
        ...
        mskClusterSecrets = secretsmanager.Secret(self, "mskClusterSecrets",
            ...
            secret_name = f"AmazonMSK_/-{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-secret",
            generate_secret_string = secretsmanager.SecretStringGenerator(
                generate_string_key = "password",
                secret_string_template = '{"username": "%s"}' % parameters_copy.mskClusterUsername,
        ...
        openSearchSecrets = secretsmanager.Secret(self, "openSearchSecrets",
            ...
            secret_name = f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-openSearchSecrets",
            generate_secret_string = secretsmanager.SecretStringGenerator(),
            encryption_key = customerManagedKey
        )
...
#############       SSM Parameter Store Configurations      #############
        ...
        mskClusterPwdParamStore = ssm.StringParameter(self, "mskClusterPwdParamStore",
            parameter_name = f"blogAws-{parameters_copy.env}-mskClusterPwd-ssmParamStore",
...
#############       Logs of MSK and Apache flink Configurations      #############
        ...
        mskClusterLogGroup = logs.LogGroup(self, "mskClusterLogGroup",
            log_group_name = f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-mskClusterLogGroup",
            retention = logs.RetentionDays.ONE_WEEK,
            removal_policy = RemovalPolicy.DESTROY
        )

        apacheFlinkAppLogGroup = logs.LogGroup(self, "apacheFlinkAppLogGroup",
            log_group_name = f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-flinkAppLogGroup",
            retention = logs.RetentionDays.ONE_WEEK,
            removal_policy = RemovalPolicy.DESTROY
        )
        
        apacheFlinkAppLogStream = logs.LogStream(self, "apacheFlinkAppLogStream",
            log_stream_name = f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-flinkAppLogStream",
...
# "allow.everyone.if.no.acl.found=true"
        ...
        mskClusterArnParamStore = ssm.StringParameter(self, "mskClusterArnParamStore",
            parameter_name = f"blogAws-{parameters_copy.env}-mskClusterArn-ssmParamStore",
...
#############       Output Values      #############
        ...
        CfnOutput(self, "vpcId",
            value = vpc.vpc_id,
            description = "VPC Id",
            export_name = f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-vpcId"
        )
        CfnOutput(self, "sgEc2MskClusterId",
            value = sgEc2MskCluster.security_group_id,
            description = "Security group Id of EC2 MSK cluster",
            export_name = f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-sgEc2MskClusterId"
        )
        CfnOutput(self, "sgMskClusterId",
            value = sgMskCluster.security_group_id,
            description = "Security group Id of MSK Cluster",
            export_name = f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-sgMskClusterId"
        )
        ...
        CfnOutput(self, "mskClusterName",
            ...
            export_name = f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-mskClusterName"
        )
        CfnOutput(self, "mskClusterArn",
            value = mskCluster.attr_arn,
            description = "ARN of an MSK cluster",
            export_name = f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-mskClusterArn"
        )
        ...
        CfnOutput(self, "customerManagedKeyArn",
            ...
            export_name = f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-customerManagedKeyArn"
        )
        CfnOutput(self, "mskClusterSecretsArn",
            value = mskClusterSecrets.secret_arn,
            description = "ARN of MSK cluster secrets",
            export_name = f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-mskClusterSecretsArn"
        )
        CfnOutput(self, "mskClusterSecretsName",
            value = mskClusterSecrets.secret_name,
            description = "MSK cluster secrets name",
            export_name = f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-mskClusterSecretsName"
        )
        CfnOutput(self, "flinkAppLogGroupArn",
            value = apacheFlinkAppLogGroup.log_group_arn,
            description = "Arn of an Apache Flink log group",
            export_name = f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-apacheFlinkAppLogGroupArn"
        )
        CfnOutput(self, "flinkAppLogGroupName",
            ...
            export_name = f"{parameters_copy.project}-{parameters_copy.env}-{parameters_copy.app}-apacheFlinkAppLogGroupName"
        )
